...
namespace VSConsole.PowerPack.Core
{
    internal static class ExtensionMethods
    {
        ...
        public static NormalizedSnapshotSpanCollection TranslateTo(this NormalizedSnapshotSpanCollection coll, ITextSnapshot snapshot, SpanTrackingMode spanTrackingMode)
        {
            if (coll.Count > 0 && coll[0].Snapshot != snapshot)
                return new NormalizedSnapshotSpanCollection(coll.Select(span => span.TranslateTo(snapshot, spanTrackingMode)));
            else
                return coll;
        }
        ...
        public static void ClearReadOnlyRegion(this IReadOnlyRegionEdit readOnlyRegionEdit, ref IReadOnlyRegion readOnlyRegion)
        {
            ...
            readOnlyRegion = (IReadOnlyRegion)null;
        }
        ...
        public static void Execute(this IOleCommandTarget target, Guid guidCommand, uint idCommand, object args = null)
        {
            ...
            finally
            {
                if (num != IntPtr.Zero)
                {
                    ExtensionMethods.VariantClear(num);
        ...
        {
            ExtensionMethods.Execute(target, VSConstants.VSStd2K, (uint)idCommand, args);
        }

        [DllImport("Oleaut32.dll", PreserveSig = false)]
        private extern static void VariantClear(IntPtr var);
    }
